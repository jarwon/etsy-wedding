...
var clearExisting = function clearExisting() {
	$(".pgArrow").remove();
	$(".pgButton").remove();
	etsyApp.currentPgNums = [];
	etsyApp.showLHSarrow = false;
...
var chkPgNums = function chkPgNums(totPgs) {
	if (totPgs <= 5) {
...
var genPgNumOptionsDisplay = function genPgNumOptionsDisplay(currentNum, totPgs) {
...
var createScreenButtons = function createScreenButtons(pgNumArray) {
	...
	var _loop = function _loop() {
		...
		var theButtonNum = pgNumArray[i];

		// console.log('selected pg', etsyApp.selectedPg);
		// console.log('button num', pgNumArray[i]);

		if (pgNumArray[i] === etsyApp.selectedPg) {
			pgButton = $("<button>").addClass("pgButton currentPg").text(theButtonNum);
		} else {
			pgButton = $("<button>").addClass("pgButton").text(theButtonNum);
		}
...
etsyApp.getCategory = function (lat, lon, userInputLocation, currentPg) {
	...
	$.ajax({
		...
		var _loop2 = function _loop2(i) {
			itemListingID = etsyApp.subCategoryListings[i].listing_id;
			...
			$.ajax({
				...
				var catTitle = etsyApp.subCategoryListings[i].title;
		...
		for (var i = 0; i < etsyApp.subCategoryListings.length; i++) {
			var itemListingID;
...
etsyApp.getPriceRange = function () {
	...
	$(".priceRange").on("submit", function (e) {
		...
		var subCategoryListings = etsyApp.subCategoryListings;
